...
static const uint16_t NDS_TO_CODEPAGE_400[2388] = {
...
void nds_text_to_ucs2(char16_t *dst, char16_t *src, size_t size) {
	for(size_t i = 0; i < size; ++i) {
		...
		if(src[i] <= 0x1e5) {
			dst[i] = NDS_TO_CODEPAGE[src[i]];
		} else if(src[i] >= 0x400 && src[i] <= 0xd54) {
			dst[i] = NDS_TO_CODEPAGE_400[src[i] - 0x400];
		}
	}
}
void ucs2_to_nds_text(char16_t *dst, char16_t *src, size_t size) {
	for(size_t i = 0; i < size; ++i) {
		...
		for(size_t j = 0; j < 485; ++j) {
			if(src[i] == NDS_TO_CODEPAGE[j]) {
				dst[i] = j;
				//ooga booga!
				goto continue_main_loop;
			}
		}
		for(size_t j = 0; j < 2388; ++j) {
			if(src[i] == NDS_TO_CODEPAGE_400[j]) {
				dst[i] = j + 0x400;
				break;
			}
		}
...
typedef struct {
	...
	size_t big_start;
	size_t big_size;
	size_t small_footer_start;
	size_t big_footer_start;
	size_t footer_size;
} nds_block_data_t;

static const nds_block_data_t NDS_DP = { NDS_DP_SMALL_BLOCK_START,
		NDS_DP_SMALL_BLOCK_LENGTH, NDS_DP_BIG_BLOCK_START,
		NDS_DP_BIG_BLOCK_LENGTH, NDS_DP_SMALL_BLOCK_FOOTER,
		NDS_DP_BIG_BLOCK_FOOTER, NDS_FOOTER_LENGTH };

static const nds_block_data_t NDS_PLAT = { NDS_PLAT_SMALL_BLOCK_START,
		NDS_PLAT_SMALL_BLOCK_LENGTH, NDS_PLAT_BIG_BLOCK_START,
		NDS_PLAT_BIG_BLOCK_LENGTH, NDS_PLAT_SMALL_BLOCK_FOOTER,
		NDS_PLAT_BIG_BLOCK_FOOTER, NDS_FOOTER_LENGTH };

static const nds_block_data_t NDS_HGSS = { NDS_HGSS_SMALL_BLOCK_START,
		NDS_HGSS_SMALL_BLOCK_LENGTH, NDS_HGSS_BIG_BLOCK_START,
		NDS_HGSS_BIG_BLOCK_LENGTH, NDS_HGSS_SMALL_BLOCK_FOOTER,
		NDS_HGSS_BIG_BLOCK_FOOTER, NDS_FOOTER_LENGTH };
...
nds_bdat_t nds_get_bdat(const uint8_t *ptr) {
	...
	bdat.block[0] = nds_get_bptr(ptr, bdat.index);
	bdat.block[1] = nds_get_bptr(ptr + NDS_ONESAVE_LENGTH, bdat.index);
	return bdat;
}

nds_sdat_t *nds_get_sdat(const nds_save_t *save, const nds_bdat_t bdat) {
	nds_sdat_t *sdat = malloc(sizeof(nds_sdat_t));
	sdat->index = bdat.index;
	sdat->block = nds_get_bptr(save->data, bdat.index);
	return sdat;
}

nds_save_index_t nds_get_main_save_index(const nds_bdat_t bdat) {
...
nds_save_t *nds_read_save_internal(nds_bdat_t bdat, nds_save_index_t index) {
	...
	memcpy(save->data, bdat.block[index.small].small, bdat.index.small_size);
	memcpy(save->data + bdat.index.big_start, bdat.block[index.big].big, bdat.index.big_size);
...
enum {
	NDS_DP_HGSS_PARTY_START = 0x94,
	NDS_PLAT_PARTY_START = 0x9C,

	NDS_DP_SIGNATURE_START = 0x5904,
	NDS_PLAT_SIGNATURE_START = 0x5BA8,
	NDS_HGSS_SIGNATURE_START = 0x4538,

	NDS_SIGNATURE_SIZE = 0x600
};

nds_party_t *nds_get_party(nds_save_t *save) {
